...
def client_post():
    ...
    # 更新order数据表
    with order_lock:
        database.update_sql("order_id", dict_data["order_id"],
                            "taxitest_order", dict_data)
    order_data = database.read_sql("order_id", dict_data["order_id"],
                                   "taxitest_order")
    ...
    if order_data["order_status"] == "f":  # 当乘客下车后，设置car_status为a
        with car_lock:
            car_data = {}
            car_data["car_id"] = order_data["car_id"]
            car_data["car_status"] = "a"
            database.update_sql("car_id", car_data["car_id"], "taxitest_car",
                                car_data)
        with order_lock:
            order_data["order_end_time"] = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime())
            database.update_sql("order_id", order_data["order_id"],
                                "taxitest_order", order_data)
...
def client_check():
    ...
    order_id = database.find_sql("phone_number", dict_data["phone_number"],
                                 "order_status", "r", "taxitest_order",
                                 "order_id")
    # print(order_id)
    if len(order_id) != 0:  # 如果存在未完成订单，则读取订单数据发送回去
        order_data = database.read_sql("order_id", order_id[0][0],
                                       "taxitest_order")
...
def client_create():
    ...
    # 创建新的一个订单
    order_data = {}
    car = database.read_sql("car_status", "a", "taxitest_car")
...
def client_login():
    ...
    if dict_data["login_or_create"] == True:
        ...
        if dict_account != None:
            ...
            # print(dict_account)
            a = database.write_sql("taxitest_client",
                                   dict_data)  # 写client进入数据库
            if a == True:
...
def ROS_post():
    ...
    if database.read_sql("car_id", dict_data["car_id"],
                         "taxitest_car") == None:
        ...
        if "car_status" in recv_data.keys():
            with car_lock:  #获取car锁
                database.update_sql("car_id", recv_data["car_id"],
                                    "taxitest_car", recv_data)
            return "跟新车辆状态成功"
        else:
            with car_lock:  #获取car锁
                database.update_sql("car_id", dict_data["car_id"],
                                    "taxitest_car", dict_data)
        # 查询当前车辆状态
        car_data = database.read_sql("car_id", dict_data["car_id"],
                                     "taxitest_car")
        if car_data["car_status"] == "a":
            return "当前没有订单"
        elif car_data["car_status"] == "r":
            with order_lock:  # 获取order锁
                # 跟新order表数据
                database.update_sql("car_id", order_data["car_id"],
                                    "taxitest_order", order_data)
        else:
            return "Error occurs"  # 车辆既没有在空闲状态，也不在订单状态
    # 得到客户端需要返回的数据
    orderid = database.find_sql("car_id", dict_data["car_id"], "order_status",
                                "r", "taxitest_order", "order_id")
    # print(orderid)
...
def ROS_register():
    ...
    # 查询数据库中是否有ROS.car_id
    database_ROS = database.read_sql("car_id", dict_data["car_id"],
                                     "taxitest_car")
